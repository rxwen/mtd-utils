			 ...
			 int *vol_id)
{
	...
	sprintf(file, lib->ubi_vol, i, minor_dev - 1);
	fd = open(file, O_RDONLY);
	if (fd == -1) {
		errno = ENODEV;
		return -1;
...
out_not_ubi:
	errmsg("\"%s\" has major:minor %d:%d, but this does not correspond to "
	       "any existing UBI device or volume", node, major_dev, minor_dev);
	errno = ENODEV;
	return -1;
}
